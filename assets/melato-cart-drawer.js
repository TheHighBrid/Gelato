(() => {
  ...
  function renderItems(cart) {
    ...
    container.innerHTML = cart.items.map((item) => {
      ...
      const img = imageUrl ? '<img class="cart-item__img" src="' + imageUrl + '" alt="' + escapeHtml(item.title) + '" width="88" height="110" loading="lazy">' : '';
      ...
      return '<div class="cart-item" role="listitem" data-line-key="' + escapeHtml(item.key) + '"><a href="' + itemUrl + '" class="cart-item__img-link" tabindex="-1" aria-hidden="true">' + img + '</a><div class="cart-item__info"><div class="cart-item__top"><a href="' + itemUrl + '" class="cart-item__title">' + escapeHtml(item.product_title) + '</a><button class="cart-item__remove" type="button" aria-label="Remove ' + escapeHtml(item.product_title) + '" data-remove-line>×</button></div>' + variant + savings + '<div class="cart-item__footer"><div class="qty-stepper" aria-label="Quantity for ' + escapeHtml(item.product_title) + '"><button class="qty-stepper__btn" type="button" aria-label="Decrease quantity" data-qty-change="-1">−</button><input class="qty-stepper__value" type="number" value="' + item.quantity + '" min="0" aria-label="Quantity"><button class="qty-stepper__btn" type="button" aria-label="Increase quantity" data-qty-change="1">+</button></div><span class="price">' + formatMoney(item.final_line_price) + '</span></div></div></div>';
    }).join('');
  }
