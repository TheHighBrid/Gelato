(() => {
  ...
  const itemsHost = drawer.querySelector('[data-cart-items],.cart-drawer__items');
  const initialEmptyMarkup = itemsHost?.innerHTML || '';
  ...
  const fetchCart = () => fetch('/cart.js', {headers:{Accept:'application/json'}}).then(r => {
