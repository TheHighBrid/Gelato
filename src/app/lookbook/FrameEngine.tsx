...
export default function FrameEngine(){const containerRef=useRef<HTMLDivElement>(null);const setCurrentFrame=useLookbookStore(s=>s.setCurrentFrame),setScrollProgress=useLookbookStore(s=>s.setScrollProgress);
 ...
 return <main ref={containerRef}>{FRAMES.map(frame=><section key={frame.id} id={`frame-${frame.id}`} className="frame-section" aria-label={frame.title} data-act={frame.act}><React.Suspense fallback={<FrameSkeleton config={frame}/>}><Frame config={frame}/></React.Suspense></section>)}</main>}
