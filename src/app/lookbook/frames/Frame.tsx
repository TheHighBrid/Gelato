...
import { FRAME_FALLBACK_IMAGE, type FrameConfig } from '@/lib/frames';
...
export default function Frame({config}:{config:FrameConfig}){
 ...
 const [image,setImage]=useState(config.assets.images[0]??FRAME_FALLBACK_IMAGE);
 useEffect(()=>setImage(config.assets.images[0]??FRAME_FALLBACK_IMAGE),[config.assets.images]);
 ...
 return <div ref={root} className={`frame frame--${config.codename.toLowerCase()}`} data-transition={config.transition}>
   <div className="frame__image parallax-layer" data-depth data-breathe style={{transform:magnifyMode?'scale(1.12)':'none'}}><Image src={image} alt="" fill sizes="100vw" priority={config.index<=3} unoptimized={image===FRAME_FALLBACK_IMAGE} onError={()=>setImage(FRAME_FALLBACK_IMAGE)} style={{objectFit:'cover'}}/></div>
